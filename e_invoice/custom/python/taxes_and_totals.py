...
class calculate_taxes_and_totals(object):
		...
		if not self.discount_amount_applied:
			for item in self.doc.get("items"):
				...
				if item.doctype in ['Quotation Item', 'Sales Order Item', 'Delivery Note Item', 'Sales Invoice Item', 'POS Invoice Item', 'Purchase Invoice Item', 'Purchase Order Item', 'Purchase Receipt Item']:
					...
					item.discount_amount = item.price_list_rate - item.rate
				# net rate calculation overwrite
				# item.net_rate = item.rate
				item.net_rate = item.rate - item.discount_before
				frappe.errprint(item.net_amount)
				# ended here
				if not item.qty and self.doc.get("is_return"):
					item.amount = flt(-1 * item.rate, item.precision("amount"))
				else:
					item.amount = flt(item.rate * item.qty,	item.precision("amount"))
				# net amount calculation overwrite
	...
	def calculate_net_total(self):
		self.doc.total_qty = self.doc.total = self.doc.base_total = self.doc.net_total = self.doc.base_net_total = self.doc.total_amount = 0.0
		...
		for item in self.doc.get("items"):
			...
			self.doc.total_amount += item.total_amount

		self.doc.round_floats_in(self.doc, ["total", "base_total", "net_total", "base_net_total","total_amount"])
